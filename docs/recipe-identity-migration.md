...
| --- | ---: | ---: | ---: | --- |
| Recipe create/import/default/share copy | Yes | No | Matching | Client creation sends one UUID in both columns so migrations 011 and 012 accept the same write; trusted server paths may still derive the text PK |
| Recipe update | UUID predicate | No application fallback | N/A | Structural guard keeps active row lookup on `recipe_uuid` |
| Recipe deletion | Yes | No | No | Attempt same-owner `delete_recipe(uuid)`; use an owner-scoped `recipe_uuid` table fallback only for the exact migration-011 missing-function response |
...
- Active compatibility input: removed from authenticated planner, template,
  history, share, shopping, made-state, and deletion commands. Client recipe
  creation temporarily sends the canonical UUID serialized as `recipes.id`
  alongside `recipe_uuid` so the same build remains valid on migration 011.
...
Client creation allocates exactly one UUID per logical attempt and reuses it
for optimistic identity, persistence, image association, retry reconciliation,
the returned `Recipe.id`, and temporary `legacyId`. No name-derived alias or
slug exists. The deletion adapter does not cache capability: every deletion
