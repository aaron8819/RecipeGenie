-- Recipe Genie: Multi-user Support Migration
-- Adds user_id to all tables and updates RLS policies for true multi-user isolation
...
-- Add user_id to pantry_items (need to change primary key)
ALTER TABLE pantry_items ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- For user_config, we need to restructure from single-row to per-user
ALTER TABLE user_config DROP CONSTRAINT IF EXISTS user_config_pkey;
ALTER TABLE user_config DROP CONSTRAINT IF EXISTS user_config_id_check;
ALTER TABLE user_config ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Add user_id to recipe_history
ALTER TABLE recipe_history ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Add user_id to weekly_plans (need to change primary key)
ALTER TABLE weekly_plans ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- For shopping_list, restructure from single-row to per-user
ALTER TABLE shopping_list DROP CONSTRAINT IF EXISTS shopping_list_pkey;
ALTER TABLE shopping_list DROP CONSTRAINT IF EXISTS shopping_list_id_check;
ALTER TABLE shopping_list ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- ============================================================================
-- STEP 2: UPDATE PRIMARY KEYS FOR MULTI-USER SUPPORT
-- ============================================================================

-- pantry_items: Change from (item) to (user_id, item)
ALTER TABLE pantry_items DROP CONSTRAINT IF EXISTS pantry_items_pkey;

-- weekly_plans: Change from (week_date) to (user_id, week_date)
ALTER TABLE weekly_plans DROP CONSTRAINT IF EXISTS weekly_plans_pkey;
...
DROP POLICY IF EXISTS "authenticated_full_access" ON recipes;
DROP POLICY IF EXISTS "authenticated_full_access" ON pantry_items;
DROP POLICY IF EXISTS "authenticated_full_access" ON user_config;
DROP POLICY IF EXISTS "authenticated_full_access" ON recipe_history;
DROP POLICY IF EXISTS "authenticated_full_access" ON weekly_plans;
DROP POLICY IF EXISTS "authenticated_full_access" ON shopping_list;

-- ============================================================================
-- STEP 5: CREATE NEW USER-SPECIFIC RLS POLICIES
-- ============================================================================
...
CREATE POLICY "users_own_pantry" ON pantry_items
  FOR ALL USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- User config: Users can only access their own config
CREATE POLICY "users_own_config" ON user_config
  FOR ALL USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- Recipe history: Users can only access their own history
CREATE POLICY "users_own_history" ON recipe_history
  FOR ALL USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);
...
-- Shopping list: Users can only access their own list
CREATE POLICY "users_own_shopping" ON shopping_list
  FOR ALL USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- ============================================================================
-- STEP 6: CREATE DEFAULT RECIPES FUNCTION
-- ============================================================================
...
-- ============================================================================
-- STEP 8: UPDATE PRIMARY KEY CONSTRAINTS (for new data only)
-- ============================================================================

-- For pantry_items, add composite unique constraint
CREATE UNIQUE INDEX IF NOT EXISTS idx_pantry_items_user_item ON pantry_items(user_id, item) WHERE user_id IS NOT NULL;

-- For weekly_plans, add composite unique constraint
CREATE UNIQUE INDEX IF NOT EXISTS idx_weekly_plans_user_date ON weekly_plans(user_id, week_date) WHERE user_id IS NOT NULL;

-- For user_config, add unique constraint on user_id
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_config_unique_user ON user_config(user_id) WHERE user_id IS NOT NULL;

-- For shopping_list, add unique constraint on user_id
CREATE UNIQUE INDEX IF NOT EXISTS idx_shopping_list_unique_user ON shopping_list(user_id) WHERE user_id IS NOT NULL;

-- ============================================================================
-- NOTES FOR PRODUCTION MIGRATION:
-- ============================================================================
-- 1. If you have existing data, you'll need to:
--    a. Assign existing data to a specific user: 
--       UPDATE recipes SET user_id = 'your-user-uuid' WHERE user_id IS NULL;
--    b. Or delete orphaned data:
--       DELETE FROM recipes WHERE user_id IS NULL;
--
-- 2. After backfilling, you can make user_id NOT NULL:
--    ALTER TABLE recipes ALTER COLUMN user_id SET NOT NULL;
--    (repeat for all tables)
--
-- 3. Then update primary keys properly for composite keys.
-- ============================================================================
