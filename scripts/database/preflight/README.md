...
Migration 014 uses 014_add_recipe_yield_metadata.sql. It requires the authoritative ledger to contain exactly 001 through 013, verifies that `recipes.yield_metadata` is absent, checks the hardened recipe-share RPC contract, and rejects every pending snapshot that the post-migration acceptance validator would reject. This includes complete recipe fields, instruction groups, image bounds, ingredients, quantities, packages, rational/lexeme agreement, canonical units, and yield semantics. CI mechanically runs the same accepted/rejected fixture matrix against this read-only preflight and the installed migration-014 validator to prevent drift. The preflight returns no customer rows or identifiers and rolls back unconditionally.
