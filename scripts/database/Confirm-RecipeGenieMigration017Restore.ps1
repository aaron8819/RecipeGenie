...
if ($definition.MigrationPath -ne $migrationPath) { throw 'Only migration 017 restore evidence is supported.' }
...
try {
    ...
    $stdoutPath = Join-Path $logsDirectory 'migration-017-restore-assertion.stdout.log'
    $stderrPath = Join-Path $logsDirectory 'migration-017-restore-assertion.stderr.log'
    $arguments = @(
    ...
    Assert-NativeExitCode $result 'Migration 017 disposable restore assertion'
...
Write-Output 'Migration 017 disposable restore: verified'
Write-Output 'Migration baseline: 001 through 016'
Write-Output 'Recipe/share aggregates and canonical structure: matched'
